        ...
        return Promise.race([
            ...
            if (error instanceof errors_1.TimeoutExpiredError) {
                throw errors_1.RaiseErrors.as(actor).create(errors_1.AssertionError, {
                    message: (0, io_1.d) `Waited ${this.timeout}, polling every ${this.pollingInterval}, for ${this.actual} to ${this.expectation}`,
...
function timeoutAfter(duration) {
    ...
    return {
        start: () => new Promise((resolve, reject) => {
            timeoutId = setTimeout(() => {
                clearTimeout(timeoutId);
                reject(new errors_1.TimeoutExpiredError((0, io_1.d) `Timeout of ${duration} has expired`));
            }, duration.inMilliseconds());
        }),
        stop: () => {
            clearTimeout(timeoutId);
